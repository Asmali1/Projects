...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (top < last) {
            ...
            if (rChild <= last) {
                if (order.compare(array[rChild], array[lChild]) >= 0
                        && order.compare(array[top], array[lChild]) > 0) {
                    exchangeEntries(array, top, lChild);
                    siftDown(array, lChild, last, order);
                } else if (order.compare(array[lChild], array[rChild]) > 0
                        && order.compare(array[top], array[rChild]) > 0) {
                    exchangeEntries(array, top, rChild);
                    siftDown(array, rChild, last, order);
                }
            } else {
                if (lChild <= last
                        && order.compare(array[top], array[lChild]) > 0) {
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        if (array.length > 1) {
            ...
            if (rChild <= array.length) {
                heapify(array, lChild, order);
                heapify(array, rChild, order);
            } else if (lChild <= array.length) {
                heapify(array, lChild, order);
